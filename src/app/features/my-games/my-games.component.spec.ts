...
describe('MyGamesComponent', () => {
  ...
  describe('loadMyGames', () => {
    ...
    it('should always filter out WISHLIST status', () => {
      component.loadMyGames();

      const callArgs = myGameService.getMyGames.calls.mostRecent().args;
      const statusesArg = callArgs[5] as MyGameStatus[];

      expect(statusesArg).toEqual(ALLOWED_STATUSES);
      expect(statusesArg).not.toContain('WISHLIST');
    });
  });
